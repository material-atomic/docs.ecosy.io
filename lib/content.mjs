...
export function getPackages() {
  ...
  /* Two keys, not one. `ORDER` is indexed per type, so comparing those indexes
     across types puts the first fork beside the first module — which is how
     hoapp, next-themes and flipbook ended up between core and http, and why a
     module late in its own list reads as absent. Group first, then position
     within the group. */
  const groupAt = (pkg) => {
    const at = GROUPS.findIndex((group) => group.type === pkg.meta.type);
  ...
  packages.sort(
    (a, b) => groupAt(a) - groupAt(b) || rank(a) - rank(b) || a.slug.localeCompare(b.slug),
  );
  cache = packages;
  return packages;
}
